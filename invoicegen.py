...
class Invoice:
    ...
    def generate_pdf(self):
        ...
        t.drawOn(c, logo_left, logo_bottom - logo_height / 4 - 210 - table_height)

        # draw rectangle that will contain the total
        total_rect_x = logo_width + logo_left + 1.5 * cm
        total_rect_y = logo_bottom - logo_height / 4 - 238 - table_height
        ...
        c.drawString(logo_width + logo_left + 2.5 * cm, logo_bottom - logo_height / 4 - 235 - table_height, "TOTAL")
        c.drawString(logo_width + logo_left + 8.3 * cm, logo_bottom - logo_height / 4 - 235 - table_height,
                     str(cum_total))
...
class InvoiceGeneratorGUI:
    ...
    def create_widgets(self):
        ...
        def show_selected_result(event):
            ...
            if selected_index:
                ...
                self.invoice.database()
                query = '''
                        SELECT customer_name, date FROM invoices
    ...
    def get_balance(self):
        ...
        summation = sum(
            int(self.payment_treeview.item(item, "values")[-1]) for item in self.payment_treeview.get_children())
